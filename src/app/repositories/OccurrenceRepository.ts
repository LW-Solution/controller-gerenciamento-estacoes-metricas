...
const createOccurrence = async (
  ...
  const { alertIdAlert, status_alert, measureIdMeasure } = occurrence
  console.log(alertIdAlert)
  try {
    // Busca o objeto Alert pelo ID
    const alert = await alertRepository.findOne({
      where: { id_alert: alertIdAlert.id_alert },
      relations: ["stations", "parameter_type"],
    })
    if (!alert) {
    ...
    newOccurrence.status_alert = status_alert
...
const updateOccurrence = async (
  ...
  try {
    ...
    if (updatedOccurrence.alertIdAlert) {
      existingOccurrence.alert.id_alert =
        updatedOccurrence.alertIdAlert.id_alert
    }

    // Verifique se a medida (measure) foi fornecida na atualização
    if (updatedOccurrence.measureIdMeasure) {
      existingOccurrence.measure.id_measure =
        updatedOccurrence.measureIdMeasure.id_measure
    }

    if (updatedOccurrence.status_alert) {
      existingOccurrence.status_alert = updatedOccurrence.status_alert
    }
